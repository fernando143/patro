...
func Transcribe(ctx context.Context, videoPath, apiKey string) (*meeting.TranscriptResult, error) {
	...
	chapters := make([]meeting.Chapter, 0, len(transcript.Chapters))
	for _, c := range transcript.Chapters {
		chapters = append(chapters, meeting.Chapter{
	...
	utterances := make([]meeting.Utterance, 0, len(transcript.Utterances))
	for _, u := range transcript.Utterances {
		speaker := derefStr(u.Speaker)
		if speaker == "" {
			speaker = "?"
		}
		utterances = append(utterances, meeting.Utterance{
			Speaker: speaker,
	...
	result := &meeting.TranscriptResult{
