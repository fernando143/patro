...
// build_prompt.
func BuildPrompt(existing []meeting.TopicRef, language, transcriptPath string) string {
...
func ParseAnalysis(raw, backendName string) *meeting.AnalysisResult {
	payload, err := extractJSON(raw)
	if err == nil {
		var result *meeting.AnalysisResult
	...
	return &meeting.AnalysisResult{
		...
		Summary: summary,
		Topics:  []meeting.Topic{{Slug: "general", Name: "General", Content: summary}},
	}
}
...
func parseResponse(payload map[string]any) (*meeting.AnalysisResult, error) {
	metadata, ok := payload["meeting"].(map[string]any)
	if !ok {
		return nil, fmt.Errorf("missing 'meeting' object in LeMUR response")
	}

	var actionItems []meeting.ActionItem
	if items, ok := metadata["action_items"].([]any); ok {
		for _, item := range items {
			if obj, ok := item.(map[string]any); ok {
				...
				actionItems = append(actionItems, meeting.ActionItem{Owner: owner, Task: task})
			} else if isTruthy(item) {
				actionItems = append(actionItems, meeting.ActionItem{Owner: "unassigned", Task: pyStr(item)})
			}
		}
	}

	var topics []meeting.Topic
	if list, ok := payload["topics"].([]any); ok {
		for _, item := range list {
			...
			topics = append(topics, meeting.Topic{Slug: slug, Name: name, Content: content})
		}
	}

	title := "Untitled meeting"
	if isTruthy(metadata["title"]) {
		title = pyStr(metadata["title"])
	}
	summary := ""
	if isTruthy(metadata["summary"]) {
		summary = pyStr(metadata["summary"])
	}

	return &meeting.AnalysisResult{
		Title:       title,
		Summary:     summary,
		KeyPoints:   asStrList(metadata["key_points"]),
		Decisions:   asStrList(metadata["decisions"]),
