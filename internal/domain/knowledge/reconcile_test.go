...
func (f fakeSimilarity) Nearest(context.Context, meeting.Topic) (NearestTopic, error) {
	if f.err != nil {
...
func staticDecider(same bool, err error) GrayZoneDecider {
	return func(_ context.Context, _ meeting.Topic, _ meeting.TopicRef) (bool, error) {
		return same, err
	}
}

func TestSemanticReconcilerThreeBand(t *testing.T) {
	candidate := meeting.Topic{Slug: "x-y", Name: "X Y", Content: "content"}
	existing := []meeting.TopicRef{{Slug: "react-hooks", Name: "React Hooks"}}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.decide != nil {
				decide = func(ctx context.Context, c meeting.Topic, n meeting.TopicRef) (bool, error) {
					calls++
...
func TestSemanticReconcilerNoExistingTopicsIsNewUnflagged(t *testing.T) {
	...
	res, err := r.Reconcile(context.Background(), meeting.Topic{Slug: "x-y", Name: "X Y"}, nil)
	if err != nil {
		t.Fatalf("Reconcile: %v", err)
	}
...
func TestSemanticReconcilerUsesMultiVectorStoreWhenAvailable(t *testing.T) {
	...
	res, err := r.Reconcile(context.Background(), meeting.Topic{Slug: "x-y", Name: "X Y", Content: "content"}, []meeting.TopicRef{{Slug: "react-hooks", Name: "React Hooks"}})
	if err != nil {
		t.Fatalf("Reconcile: %v", err)
	}
...
func TestSemanticReconcilerGrayZoneErrorSafeFails(t *testing.T) {
	...
	r := &SemanticReconciler{
		...
		MergeThreshold:    0.90,
		NewTopicThreshold: 0.70,
		Decide: func(context.Context, meeting.Topic, meeting.TopicRef) (bool, error) {
			calls++
	...
	res, err := r.Reconcile(context.Background(),
		meeting.Topic{Slug: "x-y", Name: "X Y"},
		[]meeting.TopicRef{{Slug: "react-hooks", Name: "React Hooks"}},
	)
	if err != nil {
		t.Fatalf("Reconcile: %v", err)
	}
...
func TestAddMeetingCtxMergeAnnotatesAndLedgers(t *testing.T) {
	...
	transcript := &meeting.TranscriptResult{ID: "t1", Text: "hi"}
	analysis := &meeting.AnalysisResult{
		Title:  "Weekly Sync",
		Topics: []meeting.Topic{{Slug: "x-y", Name: "X Y", Content: "some content"}},
	}
...
func TestExistingTopicsRecent(t *testing.T) {
	...
	want := []meeting.TopicRef{{Slug: "b", Name: "B"}, {Slug: "a", Name: "A"}}
