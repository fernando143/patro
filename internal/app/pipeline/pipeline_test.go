...
func failTranscribe(context.Context, string, *config.Config) (*meeting.TranscriptResult, error) {
	return nil, errBoom
}

func failAnalyze(context.Context, *meeting.TranscriptResult, []meeting.TopicRef) (*meeting.AnalysisResult, error) {
	return nil, errBoom
}
...
func TestProcessVideoRebuildsSearchIndexAfterEachVideo(t *testing.T) {
	...
	firstAnalysis := func(context.Context, *meeting.TranscriptResult, []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		return &meeting.AnalysisResult{
			Title:   "First planning session",
			Summary: "alphaonly",
			Topics:  []meeting.Topic{{Slug: "first-topic", Name: "First topic", Content: "First searchable detail."}},
		}, nil
	}
	...
	secondAnalysis := func(context.Context, *meeting.TranscriptResult, []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		return &meeting.AnalysisResult{
			Title:   "Second launch review",
			Summary: "betaonly",
			Topics:  []meeting.Topic{{Slug: "second-topic", Name: "Second topic", Content: "Second searchable detail."}},
		}, nil
	}
...
func TestProcessVideoSkipsAlreadyProcessed(t *testing.T) {
	...
	called := false
	tf := func(context.Context, string, *config.Config) (*meeting.TranscriptResult, error) {
		called = true
		return nil, errBoom
	}
...
func TestProcessVideoTranscribeError(t *testing.T) {
	...
	called := false
	af := func(context.Context, *meeting.TranscriptResult, []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		called = true
		return nil, errBoom
	}
...
func TestProcessVideoTrackerStageTransitions(t *testing.T) {
	...
	tf := func(ctx context.Context, videoPath string, c *config.Config) (*meeting.TranscriptResult, error) {
	...
	af := func(ctx context.Context, tr *meeting.TranscriptResult, existing []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		snap, err := status.Read(stateDir)
		if err != nil || snap == nil || snap.Current == nil {
...
func TestProcessVideoPassesExistingTopics(t *testing.T) {
	...
	var gotExisting []meeting.TopicRef
	af := func(ctx context.Context, tr *meeting.TranscriptResult, existing []meeting.TopicRef) (*meeting.AnalysisResult, error) {
...
func TestMakeAnalyzeFuncCLIBackendDelegatesToAnalyzeCLI(t *testing.T) {
	...
	result, err := af(context.Background(), &meeting.TranscriptResult{ID: "mtg1", Language: "en"}, nil)
	if err != nil {
...
func TestMakeAnalyzeFuncLemurWithoutAPIKey(t *testing.T) {
	...
	_, err := af(context.Background(), &meeting.TranscriptResult{ID: "t1", Language: "en"}, nil)
	if err == nil {
...
// *knowledge.SemanticReconciler with a document representer, a multi-vector
...
func TestNewReconcilerValidBackendWiresSemanticReconciler(t *testing.T) {
	...
	r := newReconciler(cfg)
	sr, ok := r.(*knowledge.SemanticReconciler)
	if !ok {
		t.Fatalf("newReconciler(cybertron) = %#v (%T), want *knowledge.SemanticReconciler", r, r)
	}
...
func TestNewReconcilerUsesClaudePathForClaudeAnalyzerBackend(t *testing.T) {
	...
	same, err := sr.Decide(context.Background(), meeting.Topic{Name: "a"}, meeting.TopicRef{Name: "b", Slug: "b"})
	if err != nil {
...
func TestMockAnalyzeShape(t *testing.T) {
	tr := &meeting.TranscriptResult{ID: "mock-abc123"}
