...
func spyAnalyze(calls *[]spyCall, title string) AnalyzeFunc {
	return func(_ context.Context, t *meeting.TranscriptResult, existing []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		*calls = append(*calls, spyCall{transcriptID: t.ID, existing: existing})
		return &meeting.AnalysisResult{Title: title, Summary: "regenerated"}, nil
	}
}

type spyCall struct {
	transcriptID string
	existing     []meeting.TopicRef
}
...
func TestRegenerateExternalTranscriptCopiedBeforeAnalysis(t *testing.T) {
	...
	af := func(_ context.Context, t *meeting.TranscriptResult, existing []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		matches, _ := filepath.Glob(filepath.Join(lib.TranscriptsDir, "*.txt"))
		copyExistedAtAnalyzeTime = len(matches) == 1
		return &meeting.AnalysisResult{Title: "External Analysis"}, nil
	}
...
func TestRegenerateMissingDateNoPriorNoteFailsWithNoFileWritten(t *testing.T) {
	...
	af := func(context.Context, *meeting.TranscriptResult, []meeting.TopicRef) (*meeting.AnalysisResult, error) {
		called = true
		return &meeting.AnalysisResult{Title: "Should not happen"}, nil
	}
